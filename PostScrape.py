...
#1. retrieve results on each individual job posting 
flat_all_jobs = pd.read_csv("get_links_output.csv")["urls"]
flat_all_jobs = list(flat_all_jobs)
#len(flat_all_jobs) #Check length of number of jobs to search on

search_output = [search_link(i) for i in show_progress(flat_all_jobs, 10)]

#search_link(i)
#2. clean output 
#search_output #prints a large mess. 

search_output_clean = [i for i in search_output if isinstance(i, dict)]
len(search_output)
len(search_output_clean)
...
jobs_df.to_csv("linkedin_data_nov13.csv", index=False)
...
#jobs_df
jobs_df.to_csv("jobs_df_clean.csv", index=False)
...
skills_summary_df.to_csv("skills_summary_df.csv", index=False)